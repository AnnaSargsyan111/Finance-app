...
### QA-001 [Minor, FIXED - verified] Portfolio ACTUAL weights can exceed the risk cap with no warning
...
### QA-003 [Minor, FIXED - verified] Visiting /auth while already logged in does not redirect away from the sign-up/login form
...
### QA-004 [Minor, FIXED - verified] Personal Finance year selector is clipped/unreadable at 375px mobile width
...
### QA-005 [Minor, NOT REPRODUCIBLE any more - verified] Login error message does not appear when the form is submitted with Enter (only with a mouse click)
...
## Fixes verified 2026-09-26

- **QA-001 fixed.** `recommendPortfolio` now adds warning `POSITION_CAP_EXCEEDED_BY_ROUNDING` (naming each holding and its actual %) whenever whole-share rounding leaves a holding above the per-position cap (`holdingsOverCap` in src/invest/allocation.ts; skipped when the cap cannot hold at all, which POSITION_CAP_NOT_ENFORCEABLE already covers). Tests: tests/quant.test.ts (helper, incl. a constructed rounding case) and tests/invest.test.ts (sweep of risk x budget: warning present exactly when an actual weight is over the cap; the sweep hit the warning path once).
- **QA-003 fixed.** AuthPage probes the session once on arrival and redirects to `next` or /personal-finance. The reset-password link (mode=reset) is exempt so it keeps working whatever the session. Browser-verified: logged in -> /auth?mode=signup lands on /personal-finance; mode=reset stays on the reset form; logged out -> no redirect loop.
- **QA-004 fixed.** pf.module.css: the year select has a fixed width (78px) that always fits four digits, and at <=480px the row uses a smaller gap, 13px text and tighter padding so prev | month | year | next stay on one row. Browser-verified at 375px: September and 2026 fully readable, no sideways scroll.
- **QA-005 no longer reproduces.** Re-tested with real key events (type wrong credentials, press Enter): the message "Email or password is incorrect." is shown, the email is kept and the password field is cleared - same as a click. The earlier always-clickable-button / validation rework of the auth forms resolved it. Sign-up and the investment-amount step were not re-tested for this specific gap.
